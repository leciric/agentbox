...
<p align="center">
  ...
  <a href="LICENSE"><img alt="License: FSL-1.1-ALv2" src="https://img.shields.io/badge/license-FSL--1.1--ALv2-blue.svg"></a>
...
Copyright (c) 2026 Leandro Ciric.

AgentBox is fair source, licensed under the [Functional Source License, version 1.1, ALv2 Future
License](LICENSE) (FSL-1.1-ALv2). You can use, modify and redistribute it for any purpose other than
building a competing product or service; each version becomes available under the Apache License,
Version 2.0 two years after its release. See [fsl.software](https://fsl.software) for what that
means in practice.

The exception is [`hubapi/`](hubapi/), the protocol AgentBox speaks to a hub, which is under the
[Apache License 2.0](hubapi/LICENSE). The hub is a separate program in a private repository, and it
keeps a copy of `hubapi/`; a permissive licence lets that copy live there without pulling the hub
under the FSL, and lets anyone else write a hub or a client that speaks the same protocol.

Commercial services built on AgentBox, such as a hub, are separate programs that talk to AgentBox
over its API; they are not part of this repository and aren't under the FSL.

Contributions are accepted under the [Developer Certificate of Origin](DCO.md): see
[CONTRIBUTING.md](CONTRIBUTING.md#license-and-the-dco).
