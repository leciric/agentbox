...
which is private: it stays closed while the rest is fair source (D44).
...
`hubapi/` is under Apache-2.0 ([`hubapi/LICENSE`](hubapi/LICENSE)) while the rest of the repository is
FSL-1.1-ALv2, which restricts competing uses; Apache-2.0 keeps the protocol itself unrestricted, so
the hub's copy of it, and anyone else's hub or client, isn't bound by the FSL. Keep it that way:
`hubapi/` must never import anything from `internal/` or the rest of this module.
